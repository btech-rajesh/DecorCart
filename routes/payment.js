...
const request = require('request');//for payment request
const jsSHA = require('jssha');//this is used npm package for payment SHA-3
const {v4:uuid} = require('uuid')
const {isLoggedIn} = require('../middleware')


router.post('/payment_gateway/payumoney', isLoggedIn, (req, res) => {
    req.body.txnid = uuid();//Here pass txnid and it should be different on every call and for product
    req.body.email = req.user.email;
    req.body.firstname = req.user.username; //Here save all the details in pay object 
    
    const pay = req.body;
 
    //put in conflict file our process.env file
